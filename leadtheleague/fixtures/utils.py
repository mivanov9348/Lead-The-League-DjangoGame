...
def generate_all_league_fixtures(season):
    print(f"Starting fixture generation for season: {season}")  # Debug: input

    league_seasons = LeagueSeason.objects.filter(season=season)
    print(f"Found league seasons: {list(league_seasons)}")  # Debug: found seasons

    if not league_seasons.exists():
        print("No LeagueSeason instances found.")  # Debug: if no seasons
        return f"No LeagueSeason instances found for the season: {season}."
    ...
    with transaction.atomic():
        for league_season in league_seasons:
            try:
                print(f"Processing LeagueSeason: {league_season}")  # Debug: current state
                generate_league_fixtures_for_season(league_season)
            except Exception as e:
                error_message = f"Error generating fixtures for {league_season}: {str(e)}"
                print(error_message)  # Debug: full error info
                errors.append(error_message)
...
def generate_league_fixtures_for_season(league_season):
    ...
    for round_num in range(total_rounds):
        ...
        first_half_schedule.append(round_pairs)
        teams = [teams[0]] + teams[-1:] + teams[1:-1]  # Rotate teams

    # Генериране на втори полусезон (обратни срещи)
    second_half_schedule = [
        [(away, home) for home, away in round_pairs]
        for round_pairs in first_half_schedule
    ]

    # Комбиниране на графиците
    full_schedule = first_half_schedule + second_half_schedule

    # Проверка за налични дати
    league_match_schedule = MatchSchedule.objects.filter(
        season=league_season.season,
        event_type='league',
    ).order_by('date')
        ...
        for home_team, away_team in fixture_round:
            bulk_create_list.append(
                LeagueFixture(
                    ...
                    league_season=league_season,
                    fixture_number=fixture_number,
                    match_time=match_date.season.match_time,  # Use season match time
                )
            )
            fixture_number += 1
